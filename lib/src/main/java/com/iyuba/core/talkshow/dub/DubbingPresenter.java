...
//                Timber.e("下载字节数____________"+progress+"总字节数____________"+mFileLength);
                ...
                if (getMvpView() != null) {
                    mMsg = MessageFormat.format(
                            ((Context) getMvpView()).getString(R.string.video_loading_tip), df.format(v));
                    Timber.e("下载字" + mMsg + "percent" + percent);
                }
                if (mContext != null) {
                    mMsg = MessageFormat.format(
                            (mContext).getString(R.string.video_loading_tip), df.format(v));
                    Timber.e("下载字" + mMsg + "percent" + percent);
                }
            }
        }
...
//        RxUtil.unsubscribe(mDelete1RecordSub);
    ...
    public void init(TalkLesson voa) {
        this.mVoa = voa;
        mDir = StorageUtil
                .getMediaDir((Context) getMvpView(), voa.voaId())
                .getAbsolutePath();
        Timber.e("下载路径" + mDir);
    }
