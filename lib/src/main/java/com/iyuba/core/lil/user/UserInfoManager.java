...
import com.iyuba.core.lil.util.GsonUtil;
...
public class UserInfoManager {
    ...
    private SharedPreferences getPreferences(){
        if (preferences==null){
            preferences = SPUtil.getPreferences(ResLibUtil.getInstance().getContext(), TAG);
        }
        return preferences;
    }
    ...
    public String getUserPic(){
        ...
        return "http://api."+ Constant.IYUBA_COM+"v2/api.iyuba?protocol=10005&uid=" + getUserId() + "&size=big";
    }
    ...
    private void refreshUserInfo(int userId,User_info info){
        ...
        String extraMergeUrl = "http://" + Constant.userSpeech + "test/merge/";
        IHeadline.setExtraMergeAudioUrl(extraMergeUrl);
        String extraUrl = "http://" + Constant.userSpeech + "test/ai/";
    ...
    //初始化视频数据
    public void initHeadlineVideoInfo(){
        IHeadline.resetMseUrl();
        String extraMergeUrl = "http://" + Constant.userSpeech + "test/merge/";
        IHeadline.setExtraMergeAudioUrl(extraMergeUrl);
        String extraUrl = "http://" + Constant.userSpeech + "test/ai/";
    ...
    public void saveAccountAndPwd(String userName,String password){
        ...
        UserManager.getInstance().setPassword("");
    ...
    public String getLoginAccount(){
        ...
        if (TextUtils.isEmpty(account)){
            account = UserManager.getInstance().getUserName();
        }
        return account;
    }
    ...
    public String getLoginPassword(){
        ...
        if (TextUtils.isEmpty(password)){
            password = UserManager.getInstance().getPassword();
        }
        return password;
    }
    ...
    public void getRemoteUserInfo(int userId, UserinfoCallbackListener listener){
        /*destroyUserInfo();
        LibRetrofitUtil.getInstance().getUserInfo(userId)
                ...
                .subscribe(new Observer<User_info>() {
                    ...
                    public void onNext(User_info info) {
                        if (info!=null){
                            Log.d("用户登录", "用户信息20001-成功-准备展示数据");
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String sign = LibEncodeUtil.md5(signStr);
...
//                        e.printStackTrace();
    ...
    public void postRemoteAccountLogin(String account, String password, UserinfoCallbackListener listener){
        /*destroyAccountLogin();
        LibRetrofitUtil.getInstance().loginByAccount(account, password)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<Login_account>() {
                    ...
                    public void onNext(Login_account bean) {
                        ...
                        if (bean!=null&&bean.getUid()!=0){
                            ...
                            if (listener!=null){
                                listener.onFail(UserInfoErrorMsg.showLoginMsg(bean.getResult()));
                            }
                        }
                    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String format = "json";
                String sign = LibEncodeUtil.md5(protocol+account+ LibEncodeUtil.md5(password)+"iyubaV2");
                String userName = LibEncodeUtil.encode(account);
                String userPsd = LibEncodeUtil.md5(password);
                ...
                try {
                    ...
                    //将数据保存在本地
                    String resStr = response.toString();
                    Login_account bean = GsonUtils.toObject(resStr,Login_account.class);
                    if (bean!=null&&bean.getUid()!=0){
                        Log.d("用户登录", "账号登录-成功-准备获取20001");
