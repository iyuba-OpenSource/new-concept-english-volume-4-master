	...
	private void playVideo() {// finish是否从头开始
		...
		if (source == 0) {
			if (SettingConfig.Instance().isHighSpeed()) {
				append = "http://staticvip." + Constant.IYUBA_CN + "sounds/voa";
			} else {
				append = "http://"+Constant.staticStr + Constant.IYUBA_CN + "sounds/voa";
			}
			url = append + newsArrayList.get(listPosition).musicUrl;
		} else if (source == 1) {
			if (SettingConfig.Instance().isHighSpeed()) {
				append = "http://staticvip." + Constant.IYUBA_CN + "sounds/minutes/";
			} else {
				append = "http://"+Constant.staticStr + Constant.IYUBA_CN + "sounds/minutes/";
			}
			url = append + newsArrayList.get(listPosition).musicUrl;
		} else if (source == 2) {
			if (SettingConfig.Instance().isHighSpeed()) {
				append = "http://staticvip." + Constant.IYUBA_CN + "sounds/song/";
			} else {
				append = "http://"+Constant.staticStr + Constant.IYUBA_CN + "sounds/song/";
			}
			url = append + newsArrayList.get(listPosition).musicUrl;
		}
...
//		});
		...
		vv.setOnCompletionListener(new IMediaPlayer.OnCompletionListener() {
			@Override
	...
	private String getDetailUrl() {
		String url = null;
		if (source == 1) {
			url = "http://apps." + Constant.IYUBA_CN + "minutes/textApi.jsp?bbcid="
					+ newsArrayList.get(listPosition).id;
		} else if (source == 0) {
			url = "http://apps." + Constant.IYUBA_CN + Constant.IYUBA+"/textNewApi.jsp?voaid="
					+ newsArrayList.get(listPosition).id;
		} else if (source == 2) {
			url = "http://apps." + Constant.IYUBA_CN + "afterclass/getText.jsp?SongId="
					+ newsArrayList.get(listPosition).id;
		}
		return url;
	}
