...
public class DemoHelper implements IIdentifierListener {
    ...
    /**
     * com.iyuba.concept2.cert.pem
     * com.iyuba.englishfm.cert.pem
     * */
    private String getRealAssetFile(Context context){
    ...
    public void getDeviceIds(Context cxt,boolean isGetOAID,boolean isGetVAID,boolean isGetAAID){
        ...
        if(!isCertInit){ // 证书只需初始化一次
            ...
            try {
                isCertInit = MdidSdkHelper.InitCert(cxt, loadPemFromAssetFile(cxt, getRealAssetFile(cxt)));
            } catch (Error e) {
                e.printStackTrace();
            }
            if(!isCertInit){
