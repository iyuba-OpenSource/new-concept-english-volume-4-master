 ...
 * 原文播放的听力学习报告
 */
class ListenStudyReportThread(val voaId:Int, private val currParagraph:Int, val startTime:String,val endTime:String, val testWords:Int, val end:Boolean, val isListen:Boolean, val showReport: Boolean) :Runnable {
                    ...
                    if (data.result=="1"){
                        if (price>0){
                            ...
                            if (showReport){
                                ...
                                EventBus.getDefault().post(ConceptBgPlayEvent(ConceptBgPlayEvent.event_audio_switch))
                            }
                        }
                    }else{
                        //直接刷新下一个
