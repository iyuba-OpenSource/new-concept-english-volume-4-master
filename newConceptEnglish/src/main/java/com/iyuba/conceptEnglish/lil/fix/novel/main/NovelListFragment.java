package com.iyuba.conceptEnglish.lil.fix.novel.main;
...
public class NovelListFragment extends BaseViewBindingFragment<FragmentListRefreshBinding> implements LessonView {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        //先查询本地数据，没有查询网络数据
        //延迟1s加载
        RxTimer.getInstance().timerInMain("novelDelayTime", 1000L, new RxTimer.RxActionListener() {
            @Override
            public void onAction(long number) {
                RxTimer.getInstance().cancelTimer("novelDelayTime");
    ...
    private void initList(){
        ...
        listAdapter.setListener(new OnSimpleClickListener<Pair<Integer, BookChapterBean>>() {
            @Override
    ...
    private void initClick(){
        binding.reBottom.setOnClickListener(v->{
            ...
            StudyActivity.start(getActivity(),chapterBean.getTypes(),chapterBean.getBookId(),chapterBean.getVoaId(),NovelBgPlaySession.getInstance().getPlayPosition());
        });
        binding.imgPlay.setOnClickListener(v->{
            ExoPlayer exoPlayer = NovelBgPlayManager.getInstance().getPlayService().getPlayer();
            if (exoPlayer!=null){
    ...
    public void showData(List<BookChapterBean> list) {
        if (list==null){
            ...
            //保存在临时数据中
            NovelBgPlaySession.getInstance().setVoaList(list);
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onPlayEvent(NovelBgPlayEvent event){
        //加载完成
        if (event.getShowType().equals(NovelBgPlayEvent.event_audio_prepareFinish)){
            if (!isToOtherPage){
                EventBus.getDefault().post(new NovelBgPlayEvent(NovelBgPlayEvent.event_control_play));
            }
        }
        ...
        //播放
        if (event.getShowType().equals(NovelBgPlayEvent.event_control_play)){
            ExoPlayer exoPlayer = NovelBgPlayManager.getInstance().getPlayService().getPlayer();
        ...
        //暂停
        if (event.getShowType().equals(NovelBgPlayEvent.event_control_pause)){
            //暂停播放
            if (NovelBgPlayManager.getInstance().getPlayService()!=null){
                ExoPlayer exoPlayer = NovelBgPlayManager.getInstance().getPlayService().getPlayer();
                if (exoPlayer!=null&&exoPlayer.isPlaying()){
                    exoPlayer.pause();
                }
        ...
        //隐藏控制栏
        if (event.getShowType().equals(NovelBgPlayEvent.event_control_hide)){
            //停止播放
            ExoPlayer exoPlayer = NovelBgPlayManager.getInstance().getPlayService().getPlayer();
            exoPlayer.pause();
            //通知栏处理
        ...
        //切换播放
        if (event.getShowType().equals(NovelBgPlayEvent.event_audio_switch)){
            //根据当前类型选择下一个音频的位置
            ExoPlayer exoPlayer = NovelBgPlayManager.getInstance().getPlayService().getPlayer();
            ...
            if (playMode==0){
                ...
                EventBus.getDefault().post(new NovelBgPlayEvent(NovelBgPlayEvent.event_audio_play));
                EventBus.getDefault().post(new NovelBgPlayEvent(NovelBgPlayEvent.event_control_play));
            }else if (playMode==1){
                //顺序播放
                if (NovelBgPlaySession.getInstance().getPlayPosition() != NovelBgPlaySession.getInstance().getVoaList().size()-1){
                    //刷新数据显示
                    int nextPosition = NovelBgPlaySession.getInstance().getPlayPosition()+1;
                    if (nextPosition>=NovelBgPlaySession.getInstance().getVoaList().size()){
//                        ToastUtil.showToast(getActivity(),"当前是最后一个了");
                        return;
                    }


                    if (isToOtherPage){
                        EventBus.getDefault().post(new NovelBgPlayEvent(NovelBgPlayEvent.event_data_refresh,nextPosition));
                    }else {
                        //获取音频并播放
                        NovelBgPlaySession.getInstance().setPlayPosition(nextPosition);
                        initPlayerAndPlayAudio(NovelBgPlaySession.getInstance().getCurData());
                    }
                }
            }else if (playMode==2){
                //随机播放
                //获取随机数
                int randomIndex = (int) (NovelBgPlaySession.getInstance().getVoaList().size()*Math.random());
                if (randomIndex == NovelBgPlaySession.getInstance().getPlayPosition()){
                    if (randomIndex == NovelBgPlaySession.getInstance().getVoaList().size()-1){
                        randomIndex--;
                    }
                    randomIndex++;
                }
                ...
                if (isToOtherPage){
                    EventBus.getDefault().post(new NovelBgPlayEvent(NovelBgPlayEvent.event_data_refresh,randomIndex));
                }else {
                    //获取音频并播放
                    NovelBgPlaySession.getInstance().setPlayPosition(randomIndex);
                    initPlayerAndPlayAudio(NovelBgPlaySession.getInstance().getCurData());
                }
            }
        }
    ...
    private void initPlayerAndPlayAudio(BookChapterBean chapterBean){
        ...
        NovelBgPlayManager.getInstance().getPlayService().getPlayer().setMediaItem(mediaItem);
        NovelBgPlayManager.getInstance().getPlayService().getPlayer().prepare();
    }
