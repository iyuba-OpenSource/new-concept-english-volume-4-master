...
import com.iyuba.conceptEnglish.lil.fix.common_fix.model.remote.bean.Junior_chapter;
...
import com.iyuba.conceptEnglish.lil.fix.common_fix.model.remote.bean.Marge_eval;
...
import com.iyuba.conceptEnglish.lil.fix.common_fix.model.remote.newService.JuniorService;
...
public class JuniorDataManager {
    ...
    //数据库-查询中小学本书籍下本章节的单词数据
    public static List<WordEntity_junior> searchWordByVoaIdFromDB(String bookId, String voaId){
        return RoomDB.getInstance().getWordJuniorDao().searchWordByVoaId(bookId,voaId);
    }

    //数据库-查询中小学本书籍下本章节的单词所属的unitId
    public static String searchWordUnitIdByVoaIdFromDB(String bookId, String voaId){
        List<WordEntity_junior> list = RoomDB.getInstance().getWordJuniorDao().searchWordByVoaId(bookId,voaId);
        if (list!=null&&list.size()>0){
            WordEntity_junior junior = list.get(0);
            return String.valueOf(junior.unit_id);
        }
        return null;
    }

    //数据库-保存中小学的单词数据
    public static List<Long> saveWordToDB(List<WordEntity_junior> list){
        return RoomDB.getInstance().getWordJuniorDao().saveData(list);
    }

    //数据库-删除中小学的单词数据
    public static void deleteWordByBookId(String bookId){
        RoomDB.getInstance().getWordJuniorDao().deleteWordByBookId(bookId);
    }
    ...
    public static Observable<Publish_eval> publishSingleEval(String bookType, String voaId, String paraId, String idIndex, int score, String content){
        ...
        String format = "json";
        int protocol = 60003;
        long uid = UserInfoManager.getInstance().getUserId();
    ...
    public static Observable<Marge_eval> margeAudioEval(String bookType, String voaId){
        ...
        List<EvalEntity_chapter> evalList = CommonDataManager.getEvalChapterByVoaIdFromDB(bookType,voaId,String.valueOf(UserInfoManager.getInstance().getUserId()));
    ...
    public static Observable<Publish_eval> publishMargeEval(String bookType,String voaId, String content){
        ...
        JuniorService apiService = RemoteManager.getInstance().createJson(JuniorService.class);
        return apiService.publishMargeAudio(topic,platform,format,protocol,uid,userName,voaId,avgScore,shuoshuotype,content,appId,rewardVersion);
    }
    ...
    public static Observable<Study_report> uploadListenStudyReportData(String types, String voaId, int uid, long startTime, long endTime, int wordCount, int endFlag){
        ...
        return service.updateReadReport(format,appId,appName,lesson,voaId,uid,device,deviceId,startDate,endDate,endFlag,wordCount,testMode,platform,testNum,sign,rewardVersion);
    }
    ...
    public static Observable<Collect_chapter> collectArticle(String types, String userId, String voaId, boolean isCollect){
        ...
        String topic = FixUtil.getLessonCollectTopic(types);
        String sentenceId = "0";
        String type = "del";
        if (isCollect){
            type = "insert";
