 ...
 * @description:
 */
public class TalkShowDetailAdapter extends RecyclerView.Adapter<TalkShowDetailAdapter.RecordHolder> {
    ...
    //界面
    private TalkShowDetailFragment fragment;

    public TalkShowDetailAdapter(TalkShowDetailPresenter presenter,TalkShowDetailFragment fragment){
    ...
    class RecordHolder extends RecyclerView.ViewHolder implements TextPage.OnSelectListener{
        ...
        public void setItem(VoaText voaText, List<VoaText> voaTextList, int pos) {
            ...
            if (pos < voaTextList.size() - 1) {//如果不是最后一个
                progressBar.setAddingTime(MessageFormat.format(itemView.getResources().getString(com.iyuba.lib.R.string.record_time),
            ...
            progressBar.setPerfectTime(MessageFormat.format(itemView.getResources().getString(com.iyuba.lib.R.string.record_time),
                    NumberUtil.keepOneDecimal(voaText.endTiming - voaText.timing)));
            progressBar.setPosition(POSITION);
            //设置默认 视图
            setDefaultView();

            tTime.setText(MessageFormat.format(itemView.getResources().getString(com.iyuba.lib.R.string.record_time),
                    NumberUtil.keepOneDecimal(voaText.endTiming - voaText.timing)));
            ...
            if (mVoaText.isIscore()) {
                ...
                if (mVoaText.getScore() >= 80) {
                    scoreText.setBackgroundResource(com.iyuba.lib.R.drawable.blue_circle);
                } else if (mVoaText.getScore() >= 45 && mVoaText.getScore() < 80) {
                    scoreText.setBackgroundResource(com.iyuba.lib.R.drawable.red_circle);
                } else {
                    scoreText.setBackgroundResource(com.iyuba.lib.R.drawable.scroe_low);
                    scoreText.setText("");
                }
            } else {
                scoreText.setVisibility(View.GONE);
            }
            if (mVoaText.isDataBase&&checkExist(voaText.getVoaId(),mTimeStamp,voaText.paraId)) {//写入数据库中记录的句子得分
                setTextColorSpan(this, voaText.words);
            }

//            if (mVoaText.isIsshowbq()) {
//                iPlay.setVisibility(View.VISIBLE);
//                tIndex.setBackgroundResource(com.iyuba.lib.R.drawable.index_green);
//            } else {
//                iPlay.setVisibility(View.GONE);
//                tIndex.setBackgroundResource(com.iyuba.lib.R.drawable.index_gray);
//            }
...
//            File file = StorageUtil.getMP4RecordFile(tIndex.getContext(), voaId, timestamp, paraId);
            File file = StorageUtil.getParaRecordMp3File(tIndex.getContext(), voaId, paraId, timestamp);
            return file.exists();
        }
        ...
        private void setCallBack(final RecordHolder recordHolder) {
            ...
            fragment.setNewScoreCallback(new TalkShowDetailFragment.NewScoreCallback() {
                @Override
        ...
        private void setTextColorSpan(RecordHolder recordHolder, List<SendEvaluateResponse.WordsBean> beans) {
            ...
            for (int i = 0; i < beans.size(); i++) {
                ...
                if (beans.get(i).getScore() < 2.5) {
                    ...
                    spannableString.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(com.iyuba.lib.R.color.GREEN)),
        ...
        private void setScoreColor(RecordHolder holder, int score) {
            if (score >= 80) {
                holder.scoreText.setBackgroundResource(com.iyuba.lib.R.drawable.blue_circle);
                holder.scoreText.setText(score + "");
                holder.mVoaText.setScore(score);
            } else if (score >= 50 && score < 80) {
                holder.scoreText.setBackgroundResource(com.iyuba.lib.R.drawable.red_circle);
                holder.scoreText.setText(score + "");
                holder.mVoaText.setScore(score);
            } else {
                holder.mVoaText.setScore(score);
                holder.scoreText.setBackgroundResource(com.iyuba.lib.R.drawable.scroe_low);
            }
            if (holder.mVoaText.isIsshowbq()) {
                holder.iPlay.setVisibility(View.VISIBLE);
                holder.tIndex.setBackgroundResource(com.iyuba.lib.R.drawable.index_green);
            }
        }
        ...
        private void initClick() {
            iPlay.setOnClickListener(v->{
                ...
                if (iPlay.getVisibility() == View.VISIBLE) {
                    ...
                    if (mOperateHolder != null) {
                        mOperateHolder.tIndex.setBackgroundResource(com.iyuba.lib.R.drawable.index_green);
                        mOperateHolder.iPause.setVisibility(View.INVISIBLE);
                        mOperateHolder.iPlay.setVisibility(View.VISIBLE);
                    }
            ...
            iRecord.setOnClickListener(v->{
                ...
                scoreText.setText("");
                scoreText.setBackgroundColor(itemView.getContext().getResources().getColor(com.iyuba.lib.R.color.white));
...
//                            itemView.getContext(), mVoaText.getVoaId(), mTimeStamp, mVoaText.paraId);
                    File saveFile = StorageUtil.getParaRecordMp3File(itemView.getContext(),mVoaText.getVoaId(),mVoaText.paraId,mTimeStamp);
...
//                mPresenter.getNetworkInterpretation(text);
                if (onSearchWordListener!=null){
                    onSearchWordListener.onSearchWord(selectText);
                }
            } else {
    ...
    private void beginUITransform() {
        ...
        mOperateHolder.scoreText.setBackgroundResource(com.iyuba.lib.R.drawable.ic_wait_64px);
    ...
    public interface OnSearchWordListener{
        void onSearchWord(String selectText);
    }
