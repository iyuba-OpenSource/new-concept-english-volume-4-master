...
public class PdfUtil {
                        ...
                        .subscribe(new Observer<Pdf_url>() {
                            ...
                            public void onNext(Pdf_url bean) {
                                if (bean.getExists().equals("true")&&!TextUtils.isEmpty(bean.getPath())){
                                    //合成链接展示
                                    String pdfUrl = "http://apps."+ Constant.IYUBA_CN+"book";
                ...
                JuniorDataManager.getLessonPdfUrl(types,downType,voaId)
                        ...
                        .subscribe(new Observer<Pdf_url>() {
                            ...
                            public void onNext(Pdf_url bean) {
                                if (bean.getExists().equals("true")&&!TextUtils.isEmpty(bean.getPath())){
                                    //合成链接展示
                                    String pdfUrl = "http://apps."+ Constant.IYUBA_CN+Constant.IYUBA;
                                    if (!bean.getPath().startsWith("/")){
                                        pdfUrl = pdfUrl+"/"+bean.getPath();
                                    }else {
                                        pdfUrl = pdfUrl+bean.getPath();
    ...
    private void showFileUrlDialog(Context context,String title,String imageUrl,String fileUrl,String voaId){
        ...
        new AlertDialog.Builder(context)
                ...
                .setPositiveButton("发送", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        sendToOther(context,title,imageUrl,fileUrl,voaId);
                    }
    ...
    //发送给其他好友
    private void sendToOther(Context context,String title,String imageUrl,String url,String voaId){
        String content = "即时导出中英课文PDF";
        if (TextUtils.isEmpty(imageUrl)){
            imageUrl = "http://app." + Constant.IYUBA_CN + "android/images/newconcept/newconcept.png";
        }

        ShareUtil.getInstance().shareUrl(context,title,content,imageUrl,url,UserInfoManager.getInstance().getUserId(),voaId);
    }
