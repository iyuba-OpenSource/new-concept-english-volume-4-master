...
public class EvalPresenter extends BasePresenter<EvalView> {
    ...
    public int getMargeAudioScore(String types, String voaId) {
        List<EvalEntity_chapter> list = CommonDataManager.getEvalChapterByVoaIdFromDB(types, voaId,String.valueOf(UserInfoManager.getInstance().getUserId()));
        int avgScore = 0;
    ...
    /*************************************获取音频评测的数量**********************/
    //获取当前章节评测的数量
    public int getCurChapterEvalCount(String types,String voaId){
        if (TextUtils.isEmpty(types)){
            return 0;
        }
