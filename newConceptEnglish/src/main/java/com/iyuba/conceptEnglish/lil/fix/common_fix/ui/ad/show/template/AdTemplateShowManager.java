package com.iyuba.conceptEnglish.lil.fix.common_fix.ui.ad.show.template;
...
public class AdTemplateShowManager {
    private static final String TAG = "AdTemplateShowManagerFix";
...
//        }
    ...
    public void loadTemplateAd(Activity context,String key,AdTemplateViewBean viewBean){
        AdRemoteManager.getTemplateAd(UserInfoManager.getInstance().getUserId(), AdShowUtil.NetParam.Flag.net_templateFlag, UserInfoManager.getInstance().isVip()?-1000:AdShowUtil.NetParam.getAdId())
    ...
    private void showAd(Activity context,String key,int[] showAdArray){
        ...
        OkHttpClient client = new OkHttpClient.Builder()
                ...
                .build();
        IyuNative iyuNative = new IyuNative(context, String.valueOf(Constant.APP_ID),client);
        ...
        // TODO: 2024/10/24 这里最新的要求将高度设置为0，这里暂时设置为0
        //设置固定数据
        int dp16px = ScreenUtil.dip2px(context, 16.0f);
        int dp100px = ScreenUtil.dip2px(context, 100.0f);
        //设置需要显示的宽高
        int adWidth = ScreenUtil.getScreenW(context);
        int adHeight = dp100px+2*dp16px;//不太清楚为啥是132px

        // TODO: 2024/10/22 这里使用mixnative-0.3.9-fix3.aar这个版本，廖洪磊要求高度传0，但是之前应该是没啥问题的，暂时设置为0吧
