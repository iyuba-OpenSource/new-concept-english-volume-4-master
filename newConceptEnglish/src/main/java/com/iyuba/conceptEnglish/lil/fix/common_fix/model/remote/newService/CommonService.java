...
import com.iyuba.conceptEnglish.lil.fix.common_fix.model.remote.bean.Report_listen;
...
public interface CommonService {
    ...
    //查询单词-搜索界面的接口
    //http://word.iyuba.cn/words/apiWord.jsp?q=dog&uid=14977580&testmode=2&appid=222
    @Headers({StrLibrary.urlPrefix+":"+UrlLibrary.HTTP_WORD,StrLibrary.urlHost+":"+NetHostManager.domain_short})
    @POST(UrlLibrary.word_search)
    Observable<Word_detail> searchWord(@Query(StrLibrary.q) String word,
    ...
    @POST("/test/ai10/")
    Observable<Word_eval> evalWord(@Body RequestBody body);
    ...
    Observable<Integral_bean> getIntegralAfterShare(@Query(StrLibrary.srid) String srid,
                                                    ...
                                                    @Query(StrLibrary.flg) String flag,
                                                    @Query(StrLibrary.uid) int uid,
                                                    @Query(StrLibrary.appid) int appId,
    ...
    Observable<CommonResponce<List<Voa>>> getHomeMocSunc(@Query(StrLibrary.book) int bookId,
